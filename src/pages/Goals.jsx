...
import PersonalizedGoalSetting from '@/components/goals/PersonalizedGoalSetting';
...
export default function Goals() {
  ...
  const { data: habits = [] } = useQuery({ queryKey: ['habits-for-goals'], queryFn: () => db.entities.Habit.filter({ active: true }) });
  const { data: studySessions = [] } = useQuery({ queryKey: ['study-for-goals'], queryFn: () => db.entities.StudySession.list('-date', 30) });
  const { data: workouts = [] } = useQuery({ queryKey: ['workouts-for-goals'], queryFn: () => db.entities.Workout.list('-date', 30) });
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-white flex items-center gap-3"><Target className="w-8 h-8 text-teal-400" />Goals & Projects</h1>
            <p className="text-slate-400">Track your ambitions and celebrate progress</p>
          </div>
          <PersonalizedGoalSetting habits={habits} studySessions={studySessions} workouts={workouts} />
        </div>
        ...
        <QuoteDisplay context="motivational" />
      </div>
